<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="Zamisao i želja organizatora MUSICA ZAGREB jest ostvarenje natjecanja za talentirane perspektivne mlade umjetnike širom svijeta koji će dobiti priliku da putem ONLINE tehnologije prezentiraju svoje umijeće, vještine i ljubav prema glazbi" /> 
        <meta name="author" content="mailto: info@zagrebmusiccompetition.com" /> 
        <meta name="language" content="Croatian" />
        <meta name="keywords" content="musica zagreb, zagreb, natjecanje, takmičenje, pjevačko, klavirsko, natjecatelji, muzičko takmičenje, grad zagreb, natjecanje mladih glazbenika, ocjenjivački sud, natjecanja, natjecatelji, Stojan Stojanov Gančev, skladbe, solo pjesme, arija, glazbene škole" />
        <meta name="robots" content="index, follow" />
        <meta name="copyright" content="Musica Zagreb">
        <link rel="icon" type="image/png" href="images/favicon.ico" />

		<title>Musica Zagreb | Muzičko Natjecanje</title>

		<!-- Font Awesome Icon -->
		<link rel="stylesheet" href="../assets/css/font-awesome.min.css">

		<!-- Custom stlylesheet -->
		<link type="text/css" rel="stylesheet" href="../assets/css/style.css"/>
        <link type="text/css" rel="stylesheet" href="../assets/css/table.css" />
    </head>
<body>
<!-- HEADER -->
<header>
    <div class="container-fluid header">
        <div class="header-filter">
        </div>
        <div class="row between">
            <div class="col-xs-12 col-md-3">
                <nav class="header-nav navigation">
                    <ul>
                        <li><a href="index.php#intro-text">Intro</a></li>
                        <li><a href="index.php#rule-info">Rules</a></li>
                        <li><a href="singingEN.php">Singing Competition</a></li>
                        <li><a href="#">Singing Competition - Results</a></li>
                        <li><a href="pianoEN.php">Piano Competition</a></li>
                        <li><a href="violinEN.php">Violin Competition</a></li>
                    </ul>
                </nav>
            </div>
            <div class="col-xs-12 col-md-9 header-title">
                <h1>Online</br>International Competition</br>of Young Musicians</br>Zagreb 2021.</h1>
            </div>
        </div>
        <div class="col-md-12 header-contact">info&#64;zagrebmusiccompetition&#46;com</div>
    </div>
</header>
<main>
    <div class="main container">
    <div class="language-select">
        <p><a href="../singingResults.php">Croatian</a> | <a href="#">English</a></p>
    </div>
    <div class="main-block">
        <div class="row center">
            <div class="col-xs-6 col-md-12">
                <div class="main-segment-title">
                    <h2>Results of the 1st Singing competition Stojan Stojanov Gančev</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="table-hold">
    <table class="neumorphic">
    <tr>
        <td>First name</td>
        <td>Last name</td>
        <td>Category</td>
        <td>Professor</td>
        <td>City</td>
        <td>Country</td>
        <td>Award</td>
        <td>Points</td>
    </tr>
    <tr>
        <td>Tasha</td>
        <td>Filipović</td>
        <td>Pretkategorija A</td>
        <td>Kristina Domović</td>
        <td>Virovitica</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>91</td>
    </tr>
    <tr>
        <td>Lorena</td>
        <td>Vlašić</td>
        <td>Pretkategorija A</td>
        <td>Kristina Domović</td>
        <td>Virovitica</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>86</td>
    </tr>
    <tr>
        <td>Denica Dena</td>
        <td>Simjanoska</td>
        <td>Pretkategorija A</td>
        <td>Gonca Bogoromova Krapovski</td>
        <td>Skopje</td>
        <td>Macedonia</td>
        <td>2.</td>
        <td>82.33</td>
    </tr>
    <tr>
        <td>Neda</td>
        <td>Dimkova</td>
        <td>Pretkategorija A</td>
        <td>Gonca Bogoromova Krapovski</td>
        <td>Skopje</td>
        <td>Macedonia</td>
        <td>3.</td>
        <td>78</td>
    </tr>
    <tr>
        <td>Katarina </td>
        <td>Evseev </td>
        <td>Pretkategorija A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>96</td>
    </tr>
    <tr>
        <td>Stipan</td>
        <td>Režić</td>
        <td>Pretkategorija A</td>
        <td>Ana Grubić Miškić</td>
        <td>Šibenik</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>98</td>
    </tr>
    <tr>
        <td>Jakov</td>
        <td>Kalajžić</td>
        <td>Pretkategorija B</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>96.66</td>
    </tr>
    <tr>
        <td>Daniel</td>
        <td>Halužan</td>
        <td>Pretkategorija B</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>94.66</td>
    </tr>
    <tr>
        <td>Dominik</td>
        <td>Šebalj</td>
        <td>Pretkategorija B</td>
        <td>Tamara Šarlija</td>
        <td>Vinkovci</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>86.33</td>
    </tr>
    <tr>
        <td>Matilda</td>
        <td>Pranjić</td>
        <td>Pretkategorija B</td>
        <td>Tamara Šarlija</td>
        <td>Vinkovci</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>90</td>
    </tr>
    <tr>
        <td>Ena</td>
        <td>Edelinski</td>
        <td>Pretkategorija B</td>
        <td>Tamara Šarlija</td>
        <td>Vinkovci</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>83</td>
    </tr>
    <tr>
        <td>Tena</td>
        <td>Maletić</td>
        <td>Pretkategorija B</td>
        <td>Ivana Garaj Korpar</td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>97.66</td>
    </tr>
    <tr>
        <td>Rea</td>
        <td>Veseli</td>
        <td>Pretkategorija B</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>99</td>
    </tr>
    <tr>
        <td>Jana</td>
        <td>Blazhevska</td>
        <td>I/A</td>
        <td>Maja Mitrovska</td>
        <td>Bitola</td>
        <td>Macedonia</td>
        <td>2.</td>
        <td>87</td>
    </tr>
    <tr>
        <td>Matea</td>
        <td>Stojanoska</td>
        <td>I/A</td>
        <td>Maja Mitrovska</td>
        <td>Prilep</td>
        <td>Macedonia</td>
        <td>3.</td>
        <td>78</td>
    </tr>
    <tr>
        <td>Iva</td>
        <td>Macan</td>
        <td>I/A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>88</td>
    </tr>
    <tr>
        <td>Magdalena</td>
        <td>Martinčić</td>
        <td>I/A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zaprešić</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>92</td>
    </tr>
    <tr>
        <td>Antonio</td>
        <td>Ceković</td>
        <td>I/A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>91.33</td>
    </tr>
    <tr>
        <td>Ružica</td>
        <td>Miletić</td>
        <td>I/A</td>
        <td>Valentina Taškova</td>
        <td>Beograd</td>
        <td>Serbia</td>
        <td>1.</td>
        <td>100</td>
    </tr>
    <tr>
        <td>Gaja</td>
        <td>Napast</td>
        <td>I/B</td>
        <td>Alenka Slokar Bajc</td>
        <td>Ljubljana</td>
        <td>Slovenia</td>
        <td>1.</td>
        <td>98.66</td>
    </tr>
    <tr>
        <td>Paula</td>
        <td>Tumir </td>
        <td>II/A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>90</td>
    </tr>
    <tr>
        <td>Sara</td>
        <td>Koljnrekaj</td>
        <td>II/A</td>
        <td>Arijana Gigliani Philipp </td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>95.33</td>
    </tr>
    <tr>
        <td>Julija</td>
        <td>Prevedan</td>
        <td>II/B</td>
        <td>Kristina Domović</td>
        <td>Virovitica</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>88.66</td>
    </tr>
    <tr>
        <td>Leona</td>
        <td>Sever</td>
        <td>II/B</td>
        <td>Kristina Domović</td>
        <td>Virovitica</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>81.33</td>
    </tr>
    <tr>
        <td>Jovana</td>
        <td>Kotevska</td>
        <td>II/B</td>
        <td>Maja Mitrovska</td>
        <td>Bitola</td>
        <td>Macedonia</td>
        <td>1.</td>
        <td>91</td>
    </tr>
    <tr>
        <td>Ana</td>
        <td>Bosert</td>
        <td>II/B</td>
        <td>Valentina Taškova</td>
        <td>Beograd</td>
        <td>Serbia</td>
        <td>1.</td>
        <td>99.33</td>
    </tr>
    <tr>
        <td>Marko</td>
        <td>Kostić</td>
        <td>II/B</td>
        <td>Valentina Taškova</td>
        <td>Beograd</td>
        <td>Serbia</td>
        <td>1.</td>
        <td>98</td>
    </tr>
    <tr>
        <td>Olivera </td>
        <td>Krivokuća</td>
        <td>II/B</td>
        <td>Valentina Taškova</td>
        <td>Beograd</td>
        <td>Serbia</td>
        <td>1.</td>
        <td>100</td>
    </tr>
    <tr>
        <td>Petra</td>
        <td>Cik</td>
        <td>III/A</td>
        <td>Helena Lucić Šego</td>
        <td>Virje</td>
        <td>Croatia</td>
        <td>2.</td>
        <td>89</td>
    </tr>
    <tr>
        <td>Laura Marincel</td>
        <td>Haller</td>
        <td>III/A</td>
        <td>Helena Lucić Šego</td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>91.3</td>
    </tr>
    <tr>
        <td>Marija</td>
        <td>Ticl</td>
        <td>III/B</td>
        <td>Marija Kuhar Šoša</td>
        <td>Požega</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>98</td>
    </tr>
    <tr>
        <td>Lada</td>
        <td>Shornik</td>
        <td>III/B</td>
        <td>Albert Pesendorfer</td>
        <td>Berlin</td>
        <td>Germany</td>
        <td>1.</td>
        <td>94</td>
    </tr>
    <tr>
        <td>Petra Pia</td>
        <td>Kartelo</td>
        <td>III/B</td>
        <td>Helena Lucić Šego</td>
        <td>Zagreb</td>
        <td>Croatia</td>
        <td>1.</td>
        <td>90</td>
    </tr>
    <tr>
        <td>Salome</td>
        <td>Makhatelashvili</td>
        <td>IV</td>
        <td>Tamaz Tseriashvili</td>
        <td>Batumi</td>
        <td>Georgia</td>
        <td>1.</td>
        <td>99</td>
    </tr>
    <tr>
        <td>Mariza</td>
        <td>Anastasiades</td>
        <td>VII</td>
        <td></td>
        <td>Nicosia</td>
        <td>Cyprus</td>
        <td>1.</td>
        <td>99</td>
    </tr>
    <tr>
        <td>Linhao</td>
        <td>Qin</td>
        <td>VII</td>
        <td>Robert Cieśla</td>
        <td>Warsaw</td>
        <td>Poland</td>
        <td>Certificate</td>
        <td>65</td>
    </tr>
    <tr>
        <td>Meitian</td>
        <td>Zhao</td>
        <td>VII</td>
        <td>Krystyna Jażwinska</td>
        <td>Warsaw</td>
        <td>Poland</td>
        <td>2.</td>
        <td>80</td>
    </tr>
    </table>
</div>
<div class="main-block">
        <div class="row center">
            <div class="col-xs-6 col-md-12">
                <div class="main-segment-title">
                    <h2>Special Awards</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="main-block">
        <div class="row center between main-segment-propositions">
            <div class="divTable">
                <div class="divTableBody">
                    <div class="divTableRow">
                        <div class="divTableCell">GRAND PRIX</div>
                        <div class="divTableCell text">
                            Salome Makhatelashvili</br>
                            Mariza Anastasiades
                        </div>
                    </div>
                    <div class="divTableRow">
                        <div class="divTableCell">Award of the Croatian Chamber Orchestra - performance with the orchestra</div>
                        <div class="divTableCell text">Marija Ticl</div>
                    </div>
                    <div class="divTableRow">
                        <div class="divTableCell">Lada Shornik and Petra Pia Kartelo Rucner Quartet Award - performance in the 2021/2022 season in the V. Lisinski Concert Hall</div>
                        <div class="divTableCell text">Lada Shornik</br>Petra Pia Kartelo</div>
                    </div>
                    <div class="divTableRow">
                        <div class="divTableCell">A special award for the overall artistic impression</div>
                        <div class="divTableCell text">Olivera Krivokuća</div>
                    </div>
                    <div class="divTableRow">
                        <div class="divTableCell">A special award for the most promising young singer</div>
                        <div class="divTableCell text">Ružica Miletić
                        </div>
                    </div>
                </div>
            </div>  
        </div>  
    </div>
    <div class="main-block">
        <div class="row center">
            <div class="col-xs-6 col-md-12">
                <div class="main-segment-title">
                    <h2>Congratulations to all the competitors and their professors on their success!</h2>
                </div>
            </div>
        </div>
    </div>
</main>

<footer>
    <div class="row center space-between footer">
        <div class="col-xs-12 col-md-6">
            <p>Contact information | E-Mail: info&#64;musiccompetitionzagreb&#46;com</p>
        </div>
        <div class="col-xs-12 col-md-6">
            <p>Website by Filip Vušković | v1.0 </p>
        </div>
    </div>
</footer>
<script src="../assets/js/scripts.js"></script>
</body>
</html>